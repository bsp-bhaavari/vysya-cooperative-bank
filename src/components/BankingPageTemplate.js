...
const BankingPageTemplate = ({ 
  ...
  const renderHeroSection = () => {
    ...
    return (
      <div className="relative overflow-hidden bg-gradient-to-br from-primary-800 via-primary-900 to-trust-950 text-white rounded-xl p-8 mb-8 border border-primary-600/40 shadow-premium-sm">
        <div className="pointer-events-none absolute inset-0 pattern-dots opacity-20" aria-hidden />
        <div className="relative flex flex-col md:flex-row items-center justify-between">
          <div className="flex-1">
            <h1 className="text-4xl font-bold mb-4 tracking-tight">{data.name}</h1>
            <p className="text-xl mb-6 text-primary-100/90 leading-relaxed">{data.description || ''}</p>
            <div className="flex flex-wrap gap-4">
              <Link 
                to="/contact" 
                className="bg-primary-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-primary-700 transition-all duration-300 shadow-md hover:-translate-y-0.5 inline-flex items-center gap-2"
              >
                Apply Now
    ...
    <div className="mb-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {(data.features || []).map((feature, index) => (
          <div key={index} className="flex items-start gap-3 p-4 bg-gradient-to-br from-surface-tint/90 to-white border border-primary-100/80 rounded-lg shadow-sm hover:shadow-soft transition-shadow duration-300">
  ...
  const renderEligibilityCriteria = () => (
    <div className="mb-8">
      ...
      <div className="bg-surface-tint border-l-4 border-primary-700 p-6 rounded-lg border border-primary-100/70">
  ...
  const renderRequiredDocuments = () => (
    <div className="mb-8">
      ...
      <div className="bg-surface-muted/80 border border-primary-100/70 p-6 rounded-lg">
        <ul className="space-y-3">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {data.interestRate && (
            <Card>
              ...
              <CardContent>
                <p className="text-2xl font-bold text-primary-800">{data.interestRate}</p>
                {data.maxAmount && (
  ...
  const renderRepaymentDetails = () => {
    ...
    return (
      <div className="mb-8">
        ...
        <div className="bg-surface-tint border-l-4 border-gold-500 p-6 rounded-lg border border-primary-100/70">
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              <p className="text-gray-600 mb-2">Maximum Amount</p>
              <p className="text-2xl font-bold text-primary-800">{data.maxAmount}</p>
            </div>
            <div>
              <p className="text-gray-600 mb-2">Maximum Tenure</p>
              <p className="text-2xl font-bold text-primary-800">{data.maxTenure}</p>
            </div>
            <div>
              <p className="text-gray-600 mb-2">Processing Time</p>
              <p className="text-2xl font-bold text-primary-800">2-5 Days</p>
            </div>
          </div>
        </div>
      </div>
  ...
  const renderCTASection = () => (
    <div className="relative overflow-hidden bg-gradient-to-br from-primary-800 via-primary-900 to-trust-950 text-white rounded-xl p-8 text-center border border-primary-600/40 shadow-premium-sm">
      ...
      <p className="text-xl mb-6 text-primary-100/90">
